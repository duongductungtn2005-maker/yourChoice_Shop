...
public class ReportService {
    ...
    public void generateAndSendAutoReport(List<String> emails, String reportType, String timeRange,
                                          LocalDateTime startTime, LocalDateTime endTime) {
        sendLogic(reportType, timeRange, startTime, endTime, emails);
    }

    private void sendLogic(String reportTitle, String timeRangeStr,
        ...
        try {
            ...
            if (responseSuccess != null && responseSuccess.getSummary() != null) {
                actualRevenue = responseSuccess.getSummary().getTotalRevenue() != null
                        ? responseSuccess.getSummary().getTotalRevenue().longValue()
                        : 0;
                successOrders = responseSuccess.getSummary().getTotalOrders() != null
                        ? responseSuccess.getSummary().getTotalOrders().intValue()
                        : 0;
            }
            ...
            filterCancel.setStatus(0);

            var responseCancel = statisticService.getRevenueStats(filterCancel);
            int cancelOrders = 0;

            if (responseCancel != null && responseCancel.getSummary() != null) {
                cancelOrders = responseCancel.getSummary().getCancelOrders() != null
                        ? responseCancel.getSummary().getCancelOrders().intValue()
                        : 0;
            }

            // 3. Lấy hóa đơn
            Page<HoaDonResponse> pageOrders =
                    hoaDonService.getOrders(null, null, null, null, start, end, PageRequest.of(0, 999999));
            List<HoaDonResponse> listOrders = pageOrders.getContent();

            // 4. Lấy thống kê sản phẩm
    ...
    private byte[] generateExcelReport(long revenue,
                                       int success,
                                       int cancel,
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            for (int i = 1; i <= cols2.length; i++) {
                sheet2.autoSizeColumn(i);
            }

            // =========================
            // SHEET 3: KHUYẾN MÃI & KHÁCH VIP
            // =========================
            Sheet sheet3 = workbook.createSheet("Khuyến Mãi & Khách VIP");

            Row titleRow3 = sheet3.createRow(1);
            titleRow3.createCell(1).setCellValue("KHUYẾN MÃI & KHÁCH HÀNG VIP");
            titleRow3.getCell(1).setCellStyle(headerStyle);

            // Khối voucher
            Row voucherTitle = sheet3.createRow(3);
            voucherTitle.createCell(1).setCellValue("Top Voucher");
            voucherTitle.getCell(1).setCellStyle(headerStyle);

            Row voucherHeader = sheet3.createRow(4);
            String[] voucherCols = {"STT", "Mã Voucher", "Tên Voucher", "Số Lần Sử Dụng", "Tổng Giảm"};
            for (int i = 0; i < voucherCols.length; i++) {
                Cell cell = voucherHeader.createCell(i + 1);
                cell.setCellValue(voucherCols[i]);
                cell.setCellStyle(headerStyle);
            }

            Row voucherEmptyRow = sheet3.createRow(5);
            voucherEmptyRow.createCell(1).setCellValue("Chưa có dữ liệu voucher.");
            voucherEmptyRow.getCell(1).setCellStyle(borderStyle);

            // Khối khách hàng
            Row customerTitle = sheet3.createRow(8);
            customerTitle.createCell(1).setCellValue("Top Khách Hàng");
            customerTitle.getCell(1).setCellStyle(headerStyle);

            Row customerHeader = sheet3.createRow(9);
            String[] customerCols = {"STT", "Mã KH", "Tên Khách Hàng", "Số Đơn", "Tổng Chi Tiêu"};
            for (int i = 0; i < customerCols.length; i++) {
                Cell cell = customerHeader.createCell(i + 1);
                cell.setCellValue(customerCols[i]);
                cell.setCellStyle(headerStyle);
            }

            Row customerEmptyRow = sheet3.createRow(10);
            customerEmptyRow.createCell(1).setCellValue("Chưa có dữ liệu khách hàng VIP.");
            customerEmptyRow.getCell(1).setCellStyle(borderStyle);
                ...
                <div style="max-width: 600px; margin: 0 auto; background: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                    ...
                    <div style="margin-top: 25px; padding: 15px; background-color: #eff6ff; border-left: 5px solid #3b82f6; border-radius: 4px;">
                        <p style="margin: 0; font-size: 14px; color: #1e3a8a; line-height: 1.5;">
                            📎 <strong>File đính kèm:</strong> Chi tiết 3 sheet Hóa Đơn, Sản Phẩm và Khuyến Mãi/Khách VIP (.xlsx) đã được đính kèm.
                        </p>
                    </div>
                </div>
            </div>

...
public class HoaDonServiceImpl implements HoaDonService {
    ...
    public HoaDonDetailResponse getOrderDetail(String maHoaDon) {
        ...
        if (hd.getHoaDonChiTiets() != null) {
            products = hd.getHoaDonChiTiets().stream().map(ct -> {
                ...
                p.setAnh(resolveProductImage(ct.getChiTietSanPham()));
                return p;
    ...
    private String resolveProductImage(ChiTietSanPham chiTietSanPham) {
        if (chiTietSanPham == null || chiTietSanPham.getHinhAnhs() == null || chiTietSanPham.getHinhAnhs().isEmpty()) {
            return null;
        }

        for (HinhAnh hinhAnh : chiTietSanPham.getHinhAnhs()) {
            if (hinhAnh != null
                    && Boolean.TRUE.equals(hinhAnh.getAnhChinh())
                    && hinhAnh.getDuongDanAnh() != null
                    && !hinhAnh.getDuongDanAnh().isBlank()) {
                return hinhAnh.getDuongDanAnh();
            }
        }

        for (HinhAnh hinhAnh : chiTietSanPham.getHinhAnhs()) {
            if (hinhAnh != null && hinhAnh.getDuongDanAnh() != null && !hinhAnh.getDuongDanAnh().isBlank()) {
                return hinhAnh.getDuongDanAnh();
            }
        }
    ...
    public void updateStatus(String maHoaDon, Integer newStatus) {
        ...
        // Trừ tồn kho khi xác nhận đơn online: Chờ xác nhận (1) -> Đã xác nhận (2)
        if (Integer.valueOf(1).equals(oldStatus) && Integer.valueOf(2).equals(newStatus)) {
            deductStockForConfirmedOrder(hd);
        }

        // Hoàn tồn kho nếu đơn đã từng trừ kho (>=2) mà chuyển về hủy hoặc chờ xác nhận
        if ((Integer.valueOf(0).equals(newStatus) || Integer.valueOf(1).equals(newStatus))
                && oldStatus != null && oldStatus >= 2) {

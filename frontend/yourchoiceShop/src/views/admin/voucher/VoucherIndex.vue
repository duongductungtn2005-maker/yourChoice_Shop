...
const getStatusLabel = (item) => {
    if (item.trangThai === 0 || isExpired(item.ngayKetThuc)) return 'Đã kết thúc'; 
    return 'Đang hoạt động';
};

const getStatusClass = (item) => {
    if (item.trangThai === 0 || isExpired(item.ngayKetThuc)) return 'badge-stopped'; 
    return 'badge-active';
};
...
const handleToggleStatus = async (item, event) => {
    ...
    if (newStatus === 0 && scope.isPrivate) {
        ...
        if (result.isConfirmed) sendEmailFlag = true; 
    } 
    else {

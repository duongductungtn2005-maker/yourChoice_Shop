      ...
      <table class="custom-table">
        ...
        <tbody>
          <tr v-for="(item, index) in list" :key="item.id" :class="{'row-disabled': item.trangThai === 0}">
            ...
            <td>
              <span class="badge" :class="item.loaiPhieu === 'CaNhan' ? 'badge-private' : 'badge-public'">
                  {{ item.loaiPhieu === 'CaNhan' ? 'Cá nhân' : 'Công khai' }}
              </span>
            </td>
...
const canReactivate = (item) => item.loaiPhieu === 'CongKhai' ? true : !isExpired(item.ngayKetThuc);
...
const toggleStatus = async (item) => {
    if (item.trangThai === 1) {
        ...
        let payload = {};
        if (item.loaiPhieu === 'CongKhai' && isExpired(item.ngayKetThuc)) {

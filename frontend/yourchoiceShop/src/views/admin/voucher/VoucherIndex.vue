        ...
        <tbody>
          <tr v-for="(item, index) in list" :key="item.id" :class="{'row-disabled': item.trangThai === 0}">
            ...
            <td class="text-center action-col">
              <!-- <button class="icon-btn" title="Xem chi tiết" @click="viewDetail(item)"><i class="fas fa-eye"></i></button> -->
...
const viewDetail = (item) => alert(item.tenPhieuGiamGia);

onMounted(fetchData);
</script>

<style scoped>
/* Giữ nguyên style bạn đã có, thêm style cho Modal */
.page-container { padding: 20px; font-family: 'Segoe UI', sans-serif; background: #f8f9fa; }

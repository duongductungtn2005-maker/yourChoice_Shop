    ...
    <div class="section-card">
      ...
      <div class="customer-layout">
        <div class="customer-info">
          <div class="avatar-circle">{{ getAvatarName(order.tenKhachHang) }}</div>
          <div class="customer-details">
            <div class="name">{{ order.tenKhachHang }}</div>
            <div class="email text-muted">Email: {{ order.emailKhachHang || 'Chưa cập nhật' }}</div>
          </div>
        </div>
        
        <div class="shipping-info" v-if="order.thongTinNhanHang">
          <div class="sub-title">Địa chỉ nhận hàng</div>
          <div class="ship-row"><i class="fas fa-user"></i> {{ order.thongTinNhanHang.tenNguoiNhan }}</div>
          <div class="ship-row"><i class="fas fa-phone"></i> {{ order.thongTinNhanHang.sdt }}</div>
          <div class="ship-row"><i class="fas fa-map-marker-alt"></i> {{ order.thongTinNhanHang.diaChi }}</div>
        </div>
        <div class="shipping-info" v-else>
          <div class="sub-title">Hình thức nhận</div>
          <div class="ship-row"><i class="fas fa-store"></i> Khách mua và nhận tại quầy</div>
        </div>
      </div>
    </div>
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      <div class="form-group">
        ...
        <input v-model="editForm.tenNguoiNhan" class="form-input" />
      </div>
      <div class="form-group">
        <label>Số điện thoại</label>
        <input v-model="editForm.sdtNguoiNhan" class="form-input" />
      </div>
      <div class="form-group">
        <label>Địa chỉ giao hàng</label>
        <input v-model="editForm.diaChiNguoiNhan" class="form-input" />
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn-secondary" @click="showEditModal = false">Hủy</button>
      <button class="btn-primary" @click="saveEdit">Lưu thay đổi</button>
    </div>
  </div>
</div>
...
<div id="invoice-print-area">
  <div class="invoice-box">
    <div class="header-row">
      <div class="shop-info">
        <h1 class="shop-name">F-SHOES STORE</h1>
        <small>Chuyên giày Sneaker chính hãng</small>
      </div>
    ...
    <div class="info-row border-top">
      <div class="col-half border-right padding-10">
        <div class="section-title">1. Từ (Người gửi):</div>
        <div class="content-text">
          <strong>Shop F-Shoes</strong><br>
          SĐT: 0988.666.888<br>
          Đ/c: FPT Polytechnic, Trịnh Văn Bô, Hà Nội
        </div>
      </div>
      
      <div class="col-half padding-10">
        <div class="section-title">2. Đến (Người nhận):</div>
        <div class="content-text">
          <strong>{{ order.thongTinNhanHang?.tenNguoiNhan || order.tenKhachHang }}</strong><br>
          SĐT: <b>{{ order.thongTinNhanHang?.sdt || '---' }}</b><br>
          Đ/c: {{ order.thongTinNhanHang?.diaChi || 'Khách nhận tại quầy' }}
        </div>
      </div>
    </div>

    <div class="content-row border-top">
      <div class="col-qr border-right padding-10 center-flex">
        <div class="section-title">3. Mã đơn hàng</div>
        <img :src="`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${order.maHoaDon}`" class="qr-img" />
        <div class="qr-text">{{ order.maHoaDon }}</div>
      </div>

      <div class="col-products padding-10">
        <div class="section-title">4. Nội dung hàng (Tổng SL: {{ order.sanPhamHoaDon?.length || 0 }})</div>
        <ul class="product-list-print">
          <li v-for="(sp, i) in order.sanPhamHoaDon" :key="i">
            - {{ sp.tenSanPham }} <br>
            <span class="product-attr">({{ sp.mauSac }} / {{ sp.size }}) x <b>{{ sp.soLuong }}</b></span>
          </li>
        </ul>
      </div>
    </div>
    ...
    <div class="footer-row border-top">
      <div class="col-half border-right padding-10">
        <div class="section-title">5. Tiền thu người nhận:</div>
        <div class="cod-price">
          {{ order.trangThai === 4 || order.lichSuThanhToan?.length > 0 ? '0 VNĐ' : formatMoney(order.tongTienSauGiam) }}
        </div>
        <div class="note-text">(Vui lòng kiểm tra kỹ trước khi nhận)</div>
      </div>

      <div class="col-half padding-10 text-center">
        <div class="section-title">6. Chữ ký người nhận:</div>
        <div class="note-text">(Xác nhận hàng nguyên vẹn)</div>
        <div class="signature-space"></div>
      </div>
    </div>
    
    <div class="print-footer">In từ hệ thống YourChoice Shop</div>
  </div>
</div>
  </div>
...
// Timeline steps (Có Icon)
const timelineSteps = [
  { status: 1, label: 'Chờ xác nhận', icon: 'fas fa-file-invoice' },
  { status: 2, label: 'Chờ giao hàng', icon: 'fas fa-box-open' },
  { status: 3, label: 'Đang giao', icon: 'fas fa-shipping-fast' },
  { status: 4, label: 'Hoàn thành', icon: 'fas fa-clipboard-check' }
]
...
onMounted(async () => {
  try {
    ...
    console.log("=== CHECK DỮ LIỆU ===");
    console.log("Toàn bộ đơn hàng:", res.data);
    console.log("Biến ngayTao:", res.data.ngayTao);
    console.log("Biến createdAt:", res.data.createdAt);
  } catch (e) {
    console.error(e)
...
const cancelOrder = async () => {
    if(!confirm('Bạn chắc chắn muốn hủy đơn hàng này? Hành động không thể hoàn tác!')) return;
    
    try {
        // Trạng thái 0 là Hủy (Theo quy ước của bạn)
        await updateOrderStatus(order.value.maHoaDon, 0);
        order.value.trangThai = 0;
        alert("Đã hủy đơn hàng thành công!");
    } catch (e) {
        console.error(e);
        alert("Lỗi khi hủy đơn: " + (e.response?.data || "Lỗi hệ thống"));
    }
}

// Helpers
const formatMoney = (val) => new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(val || 0);
// --- SỬA TRONG THẺ <SCRIPT SETUP> ---

// --- Copy đè đoạn này vào trong <script setup> của file ChiTietDonHang.vue ---

// --- Thay thế hàm formatDate cũ trong <script setup> ---

// --- Thay thế hàm formatDate trong <script setup> ---
...
const formatDate = (val) => {
  ...
  // TRƯỜNG HỢP 1: Dữ liệu là Mảng (Ví dụ: [2026, 1, 12, 13, 56])
  ...
  // TRƯỜNG HỢP 2: Dữ liệu là Chuỗi (Ví dụ: '2026-01-12T13:56... 09:00')
  else {
    // FIX LỖI QUAN TRỌNG:
    // Nếu chuỗi có khoảng trắng (ví dụ " 09:00"), thay bằng dấu "+" để thành chuẩn ISO ("+09:00")
...
const getAvatarName = (name) => name ? name.charAt(0).toUpperCase() : 'K';
const printOrder = () => window.print();
const openEdit = () => { 
    if (order.value.thongTinNhanHang) {
        // Copy dữ liệu từ màn hình vào form sửa
        editForm.value = {
            tenNguoiNhan: order.value.thongTinNhanHang.tenNguoiNhan,
            sdtNguoiNhan: order.value.thongTinNhanHang.sdt,
            diaChiNguoiNhan: order.value.thongTinNhanHang.diaChi
        }
    } else {
        editForm.value = {}
    }
...
const saveEdit = async () => { 
    try {
        await updateOrderInfo(order.value.maHoaDon, editForm.value);
        alert("Cập nhật thông tin thành công!");
        showEditModal.value = false;
        
        // Load lại trang để thấy thông tin mới
...
const statusInfo = (s) => {
    const map = {
        ...
        1: { text: 'Chờ xác nhận', class: 'badge-warn' },
        2: { text: 'Chờ giao hàng', class: 'badge-info' },
        3: { text: 'Đang giao', class: 'badge-purple' },
        4: { text: 'Hoàn thành', class: 'badge-success' }
    }
    return map[s] || { text: '?', class: '' }
}
const orderTypeInfo = (t) => {
    return t === 'Trực tuyến' ? { text: 'Online', class: 'badge-success' } : { text: 'Tại quầy', class: 'badge-info' }
}
</script>
...
.badge-cash { background: #dcfce7; color: #166534; border: 1px solid #bbf7d0; } /* Màu xanh lá tiền mặt */
.badge-transfer { background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; } /* Màu xanh dương chuyển khoản */
.payment-table { 
  width: 100%; 
  border-collapse: collapse; /* Quan trọng để bỏ khoảng hở thừa */
  table-layout: fixed; /* Giúp chia % chiều rộng chính xác */
}
/* Buttons & Badges */
.btn-primary { background: #2563eb; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; gap: 6px; }
.btn-secondary { background: white; border: 1px solid #d1d5db; padding: 8px 16px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; gap: 6px; }
.btn-action { color: white; padding: 10px 24px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; }
.btn-action.confirm { background: #f59e0b; } .btn-action.ship { background: #3b82f6; } .btn-action.complete { background: #10b981; } .btn-action.cancel { background: #ef4444; }
.badge { padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.badge-success { background: #dcfce7; color: #166534; } .badge-info { background: #e0f2fe; color: #0369a1; } .badge-warn { background: #fef3c7; color: #92400e; } .badge-purple { background: #ede9fe; color: #6d28d9; } .badge-cancel { background: #fee2e2; color: #991b1b; }
...
.form-input { width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px; }

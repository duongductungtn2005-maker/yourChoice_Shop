  ...
  <div v-if="order && showPaymentModal" class="modal-overlay" @click.self="showPaymentModal = false">
      <div class="modal payment-modal">
        ...
        <div v-if="paymentMethod === 'TRANSFER'" class="payment-content qr-section">
          ...
          <div class="qr-code">
            <img :src="`https://img.vietqr.io/image/MB-0876524519-compact2.png?amount=${order.tongTienSauGiam}&addInfo=Thanh toan don hang ${order.maHoaDon}`" alt="QR Thanh toán" />
...
const confirmPayment = async () => {
    try {
        const paymentData = {
            hinhThucThanhToan: paymentMethod.value === 'CASH' ? 'TIEN_MAT' : 'CHUYEN_KHOAN',
            soTien: order.value.tongTienSauGiam,
            ghiChu: `Thanh toán đơn hàng ${order.value.maHoaDon}`
        };

        await request.post(`/admin/orders/${orderId}/payment`, paymentData);
        
        showPaymentModal.value = false;
        toastSuccess('Thanh toán thành công!');
        
        // QUAN TRỌNG: Gọi lại hàm này để load lại bảng Lịch sử thanh toán từ Server
        await fetchOrderDetail(); 
        
    } catch (e) {
        console.error(e);
        const errorMsg = e.response?.data || 'Lỗi khi xác nhận thanh toán';
        toastError(typeof errorMsg === 'object' ? 'Lỗi hệ thống' : errorMsg);
    }
};
...
const fetchOrderDetail = async () => {
   ...
   try {
      ...
      editForm.value = {
          ...
          sdt: order.value.sdtKhachHang || order.value.thongTinNhanHang?.sdt,
...
const saveEditInfo = async () => {
    try {
        // Giả sử API update info
        await request.put(`/hoa-don/${orderId}/info`, editForm.value);
...
<style scoped>
/* General */
.page-container { padding: 20px; background: #ebecee; min-height: 100vh; font-family: 'Segoe UI', sans-serif; color: #334155; }
.loading-state { height: 300px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; color: #64748b; }
...
.sub-info { font-size: 14px; color: #64748b; margin-top: 5px; }
.divider { margin: 0 8px; color: #cbd5e1; }
.text-primary { color: #2563eb; }

/* Grid */
.detail-grid { display: grid; grid-template-columns: 2.5fr 1fr; gap: 20px; }
.card { background: #fff; border: 1px solid #bfdbfe; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); margin-bottom: 20px; overflow: hidden; }
.card-header-icon { background: #f1f5f9; padding: 12px 16px; border-bottom: 1px solid #e2e8f0; font-weight: 700; color: #334155; display: flex; align-items: center; gap: 8px; }

/* Timeline */
.timeline-wrapper { padding: 25px 20px 10px; }
.steps-container { display: flex; justify-content: space-between; position: relative; }
.steps-container::before { content: ''; position: absolute; top: 18px; left: 0; right: 0; height: 2px; background: #e2e8f0; z-index: 1; }
.step-item { flex: 1; display: flex; flex-direction: column; align-items: center; position: relative; z-index: 2; opacity: 0.6; }
.step-item.active { opacity: 1; }
.step-icon { width: 36px; height: 36px; border-radius: 50%; background: #fff; border: 2px solid #cbd5e1; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; transition: 0.3s; color: #64748b; }
.step-item.active .step-icon { background: #10b981; border-color: #10b981; color: #fff; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.2); }
.step-label { font-size: 11px; font-weight: 600; text-align: center; }

/* Info Rows */
.info-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
.info-body { padding: 16px; display: flex; flex-direction: column; gap: 10px; }
.info-line { display: flex; justify-content: space-between; font-size: 13px; }
.info-line .label { color: #64748b; min-width: 80px; }
...
.custom-table th { background:#f5f5f5 !important; color: #000000; padding: 10px; text-align: left; border-bottom: 1px solid #bfdbfe; white-space: nowrap; }
.custom-table td { padding: 10px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; color: #334155; }
.product-cell-flex { display: flex; align-items: center; gap: 8px; }
.mini-img { width: 40px; height: 40px; object-fit: cover; border-radius: 4px; border: 1px solid #e2e8f0; }
.text-right { text-align: right; }
.text-center { text-align: center; }
.badge-attr { background: #f1f5f9; padding: 2px 6px; border-radius: 4px; font-size: 11px; color: #475569; border: 1px solid #e2e8f0; }

/* Sidebar Summary */
.summary-body { padding: 16px; font-size: 13px; }
.summary-row { display: flex; justify-content: space-between; margin-bottom: 8px; }
.summary-divider { height: 1px; background: #e2e8f0; margin: 12px 0; }
.total-row { font-size: 15px; font-weight: 700; color: #0f172a; align-items: center; }
.total-price { color: #dc2626; font-size: 18px; }
.text-green { color: #16a34a; }

/* History */
.history-body { padding: 16px; max-height: 200px; overflow-y: auto; }
.history-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 12px; font-size: 12px; position: relative; }
.history-item::before { content: ''; position: absolute; left: 3px; top: 8px; bottom: -12px; width: 1px; background: #e2e8f0; }
.history-item:last-child::before { display: none; }
.history-item .dot { width: 7px; height: 7px; background: #3b82f6; border-radius: 50%; margin-top: 4px; z-index: 1; }
.h-info { flex: 1; }
.h-date { color: #64748b; font-size: 10px; display: block; margin-bottom: 2px; }
.h-desc { font-weight: 500; color: #334155; }
.h-amount { font-weight: 700; color: #0f172a; }
.history-footer { padding: 10px 16px; border-top: 1px solid #e2e8f0; text-align: right; }

/* Buttons & Badges */
.btn { height: 36px; padding: 0 16px; border-radius: 6px; font-weight: 600; cursor: pointer; border: none; display: inline-flex; align-items: center; gap: 8px; font-size: 13px; transition: 0.2s; }
...
.btn-blue-block { width: 100%; background: linear-gradient(135deg, #1e3a8a 0%, #0f172a 100%); color: #fff; ; color: white; justify-content: center; }
.btn-orange-block { width: 100%; background: #f97316; color: white; justify-content: center; }
.btn-pay { background: #10b981; color: white; border: none; padding: 6px 12px; border-radius: 4px; font-size: 12px; cursor: pointer; }
.btn-cancel { background: transparent; border: none; color: #64748b; cursor: pointer; }

.badge { padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; margin-right: 5px; }
...
.badge-payment { background: #fae8ff; color: #86198f; }
.badge-success { background: #dcfce7; color: #166534; }
.badge-cancel { background: #fee2e2; color: #991b1b; }

/* Modal */
.modal-backdrop, .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000; }
.modal-container, .modal { background: #fff; padding: 24px; border-radius: 12px; width: 400px; max-width: 95%; box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
.modal-title { margin-top: 0; }
.modal-actions { display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; }
.modal-footer-between { display: flex; justify-content: space-between; margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 15px; }
...
/* Payment Modal */
.payment-modal { width: 450px !important; padding: 20px !important; }
.modal-header-flex { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 10px; }
.modal-header-flex h3 { margin: 0; font-size: 18px; color: #334155; }
.close-btn { background: none; border: none; font-size: 24px; cursor: pointer; color: #94a3b8; }

.payment-summary { background: #f8fafc; padding: 10px 15px; border-radius: 8px; margin-bottom: 15px; }
...
.tab-item { flex: 1; padding: 8px; border: 1px solid #e2e8f0; background: white; border-radius: 6px; cursor: pointer; font-weight: 600; color: #64748b; transition: 0.3s; }
.tab-item.active { background: #10b981; color: white; border-color: #10b981; }

.qr-section { text-align: center; }
.bank-info { margin-bottom: 10px; font-size: 13px; }
.qr-code img { width: 200px; height: 200px; border: 1px solid #eee; border-radius: 8px; padding: 5px; }
.qr-hint { font-size: 12px; color: #64748b; margin-top: 5px; }

.payment-input { width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 16px; margin-top: 5px; }

.table-mini { width: 100%; border-collapse: collapse; margin: 15px 0; font-size: 12px; }
.table-mini th { background: #f1f5f9; padding: 8px; border: 1px solid #e2e8f0; }
.table-mini td { padding: 8px; border: 1px solid #e2e8f0; }

.payment-footer { border-top: 1px solid #eee; padding-top: 15px; }
.remaining-row { display: flex; justify-content: space-between; margin-bottom: 15px; font-size: 15px; }
.btn-submit-payment { width: 100%; padding: 12px; background: #2563eb; color: white; border: none; border-radius: 6px; font-weight: 700; cursor: pointer; transition: 0.3s; }
.btn-submit-payment:hover { background: #1d4ed8; }
.action-buttons-col {
  display: flex;
  flex-direction: column; /* vì button đang xếp dọc */
  gap: 10px; /* chỉnh số px theo ý bạn */
}
/* --- CSS CHO GIAO DIỆN IN --- */
@media screen {
  .print-only { display: none; } /* Ẩn template in khi đang xem trên web */
}
...
#invoice-print { font-family: 'Times New Roman', serif; line-height: 1.6; }
.invoice-header { display: flex; justify-content: space-between; border-bottom: 2px solid #333; padding-bottom: 10px; margin-bottom: 20px; }
.brand-name { color: #d32f2f; margin: 0; }
...
.invoice-table th { background: #f2f2f2 !important; font-weight: bold; }

.invoice-footer { display: flex; justify-content: space-between; margin-top: 30px; }
...
.thanks-msg { font-weight: bold; font-size: 16px; margin-top: 20px; }

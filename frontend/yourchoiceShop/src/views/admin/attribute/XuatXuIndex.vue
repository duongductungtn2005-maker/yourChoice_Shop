<template>
  <div class="attribute-page">
    <div class="header-section">
            <h1 class="page-title">Quản lý sản phẩm / Xuất xứ</h1>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="text-center" width="40%">Tên Xuất Xứ</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id">
              ...
              <td class="text-center font-medium text-primary">{{ item.tenXuatXu }}</td>
       ...
       <div class="modal-dialog-custom">
          <div class="modal-content-custom">
             <div class="modal-header-custom">
                <h3>{{ isEdit ? 'Cập nhật xuất xứ' : 'Thêm mới xuất xứ' }}</h3>
             </div>
                ...
                <form @submit.prevent="saveData">
                   <div class="form-group">
                      <input 
                        ...
                        placeholder="Nhập tên xuất xứ (VD: Việt Nam, Hàn Quốc...)" 
                        ref="nameInput"
                      >
                   </div>
...
// API URL CHO XUẤT XỨ
const API_URL = 'http://localhost:8080/api/v1/xuat-xu';
...
const exportExcel = async () => {
    const confirmRes = await Swal.fire({
        title: 'Xác nhận',
        text: 'Bạn có muốn tải xuống danh sách xuất xứ không?',
        icon: 'question',
    ...
    try {
        ...
        link.setAttribute('download', `DS_XuatXu_${dateStr}.xlsx`);
...
const openModal = (item = null) => {
    if (item) {
        ...
        form.ten = item.tenXuatXu; // Đổi thành tenXuatXu
...
const saveData = async () => {
    if (!form.ten.trim()) {
        ...
        Toast.fire({ icon: 'warning', title: 'Vui lòng nhập tên xuất xứ!' });
        return;
    }
    ...
    try {
        ...
        // Check trùng với tenXuatXu
        const isDuplicate = listCheck.some(item => 
            item.tenXuatXu.toLowerCase() === form.ten.trim().toLowerCase() && 
        ...
        if (isDuplicate) {
            Swal.fire({
                ...
                text: `Xuất xứ "${form.ten}" đã có trong hệ thống.`,
...
const handleToggleStatus = async (item, event) => {
    ...
    const result = await Swal.fire({
        ...
        text: `Bạn có chắc muốn ${actionText.toLowerCase()} xuất xứ "${item.tenXuatXu}"?`, // Dùng tenXuatXu
        icon: 'question',
        showCancelButton: true,
        ...
        try {
            if (newStatus === 0) {
                ...
                await axios.put(`${API_URL}/${item.id}`, { 
                    ten: item.tenXuatXu, // Gửi đúng tên Xuất Xứ
                    trangThai: 1 
                });
            }
...
/* Nút Outline (Màu trắng, viền đậm) -> Dùng cho nút phụ (Đóng/Không) */
...
/* Nút Solid (Màu đậm) -> Dùng cho nút chính (Lưu/Thêm/Có) */

             ...
             <div class="modal-body-custom">
                <form @submit.prevent="saveData">
                   <div class="form-group text-left">
                      <label class="form-label">Tên xuất xứ <span class="required">*</span></label>
                      <input 
                        type="text" 
                        v-model="form.ten" 
...
const exportExcel = async () => {
    const confirmRes = await Swal.fire({
        title: 'Xác nhận', text: 'Bạn có muốn tải xuống danh sách xuất xứ không?', icon: 'question',
...
const openModal = (item = null) => {
    ...
    if (item) {
        ...
        form.ten = item.tenXuatXu; // Chú ý: Backend trả về tenXuatXu
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            // LƯU Ý: Đổi 'tenChatLieu' thành 'tenXuatXu', 'tenCoAo', 'tenKichThuoc'... tương ứng với từng file
...
const handleToggleStatus = async (item, event) => {
    ...
    const result = await Swal.fire({
        ...
        text: `Bạn có chắc muốn ${actionText.toLowerCase()} "${item.tenXuatXu}"?`,
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenXuatXu, trangThai: 1 }); // Sửa tên biến tương ứng

    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal = false">
      <div class="modal-content" style="width: 450px;">
        <div class="modal-header">
          <h3 style="margin:0">{{ isEdit ? 'Cập nhật xuất xứ' : 'Thêm mới xuất xứ' }}</h3>
          ...
          <form @submit.prevent="saveData">
            <div class="form-group text-left">
              ...
              <input 
                ...
                placeholder="Nhập tên xuất xứ (VD: Việt Nam...)" 
                ref="nameInput"
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            item.tenXuatXu.toLowerCase() === form.ten.trim().toLowerCase() && item.id !== currentId.value
        );
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenXuatXu, trangThai: 1 });

    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal = false">
      <div class="modal-content" style="width: 450px;">
        <div class="modal-header">
          <h3 style="margin:0">{{ isEdit ? 'Cập nhật cổ áo' : 'Thêm mới cổ áo' }}</h3>
          ...
          <form @submit.prevent="saveData">
            <div class="form-group text-left">
              ...
              <input 
                ...
                placeholder="Nhập tên cổ áo (VD: Cổ tròn...)" 
                ref="nameInput"
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            item.tenCoAo.toLowerCase() === form.ten.trim().toLowerCase() && item.id !== currentId.value
        );
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenCoAo, trangThai: 1 });

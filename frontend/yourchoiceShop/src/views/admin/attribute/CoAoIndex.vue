        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id">
              ...
              <td class="font-medium text-primary">{{ item.tenCoAo }}</td>
...
const exportExcel = async () => {
    try {
        ...
        // Tạo link ảo và click để tải về
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        
        // Đặt tên file (DS_CoAo_YYYY-MM-DD.xlsx)
        ...
        Swal.fire({
            icon: 'error',
            title: 'Lỗi',
            text: 'Không thể xuất file Excel. Vui lòng thử lại sau.'
        });
    }
};
...
.swal-btn-outline {
    ...
    padding: 8px 24px; border-radius: 6px; font-weight: 600; cursor: pointer; margin-left: 10px;
}

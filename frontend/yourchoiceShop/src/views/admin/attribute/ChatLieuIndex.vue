      ...
      <div class="card-header">
         <div class="toolbar">
            <div class="search-wrap">
               ...
               <input type="text" v-model="filter.keyword" placeholder="Tìm kiếm chất liệu..." @keyup.enter="fetchData">
            </div>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id">
              ...
              <td class="text-center">
                ...
                <label class="switch" title="Bật/Tắt trạng thái">
                    <input type="checkbox" :checked="item.trangThai === 1" @click="handleToggleStatus(item, $event)">
          ...
          <div class="modal-content-custom">
             ...
             <div class="modal-body-custom">
                <form @submit.prevent="saveData">
                   <div class="form-group text-left"> <label class="form-label">Tên chất liệu <span class="required">*</span></label>
                      <input 
                        type="text" 
                        v-model="form.ten" 
...
import { validate } from '@/utils/validate'; // 1. Import Validate
...
const errors = reactive({ ten: null }); // 2. State lưu lỗi
...
const validateField = (field) => {
    if (field === 'ten') {
        // Check rỗng + Check độ dài tối thiểu 2 ký tự
...
// --- API ---
const fetchData = async () => { /* Giữ nguyên code cũ */
    loading.value = true;
    try {
        const res = await axios.get(API_URL, { params: { page: page.value - 1, size: pageSize.value, keyword: filter.keyword } });
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenChatLieu, trangThai: 1 }); // Sửa tên biến tương ứng
...
const visiblePages = computed(() => { let p = []; for (let i = 1; i <= totalPages.value; i++) if (i===1 || i===totalPages.value || (i>=page.value-1 && i<=page.value+1)) p.push(i); return p; });
const formatDate = (d) => d ? new Date(d).toLocaleDateString('vi-VN', {day:'2-digit',month:'2-digit',year:'numeric'}) : '';
const exportExcel = async () => { /* Giữ nguyên code exportExcel cũ */ };

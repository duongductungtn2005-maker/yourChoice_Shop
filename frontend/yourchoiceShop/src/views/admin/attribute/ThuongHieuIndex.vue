<template>
  <div class="attribute-page">
    <div class="header-section">
      <h1 class="page-title">Quản lý sản phẩm / Thương hiệu</h1>
    </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="text-center" width="40%">Tên Thương Hiệu</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id">
              ...
              <td class="text-center font-medium text-primary">{{ item.tenThuongHieu }}</td>
       ...
       <div class="modal-dialog-custom">
          <div class="modal-content-custom">
             <div class="modal-header-custom">
                <h3>{{ isEdit ? 'Cập nhật thương hiệu' : 'Thêm mới thương hiệu' }}</h3>
             </div>
...
const API_URL = 'http://localhost:8080/api/v1/thuong-hieu';
...
const exportExcel = async () => {
    const confirmRes = await Swal.fire({
        title: 'Xác nhận', text: 'Bạn có muốn tải xuống danh sách thương hiệu không?', icon: 'question',
    ...
    try {
        ...
        link.setAttribute('download', `DS_ThuongHieu_${dateStr}.xlsx`);
...
const openModal = (item = null) => {
    ...
    if (item) {
        ...
        form.ten = item.tenThuongHieu; // Chú ý: Backend trả về tenThuongHieu
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            ...
            item.tenThuongHieu.toLowerCase() === form.ten.trim().toLowerCase() && 
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenThuongHieu, trangThai: 1 }); // Sửa tên biến tương ứng

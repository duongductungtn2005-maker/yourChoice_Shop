    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal = false">
      <div class="modal-content" style="width: 450px;">
        <div class="modal-header">
          <h3 style="margin:0">{{ isEdit ? 'Cập nhật thương hiệu' : 'Thêm mới thương hiệu' }}</h3>
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            item.tenThuongHieu.toLowerCase() === form.ten.trim().toLowerCase() && item.id !== currentId.value
        );
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenThuongHieu, trangThai: 1 });

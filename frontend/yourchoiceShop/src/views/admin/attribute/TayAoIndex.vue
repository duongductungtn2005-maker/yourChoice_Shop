        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="text-center" width="40%">Tên Tay Áo</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="(item, index) in items" :key="item.id">
              ...
              <td class="text-center font-medium text-primary">{{ item.tenTayAo }}</td>
...
const openModal = (item = null) => {
    ...
    if (item) {
        ...
        form.ten = item.tenTayAo; // Chú ý: Backend trả về tenTayAo
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenTayAo, trangThai: 1 }); // Sửa tên biến tương ứng

    ...
    <div class="modal-overlay" v-if="showModal" @click.self="showModal = false">
      <div class="modal-content" style="width: 450px;">
        <div class="modal-header">
          <h3 style="margin:0">{{ isEdit ? 'Cập nhật màu sắc' : 'Thêm mới màu sắc' }}</h3>
...
const saveData = async () => {
    ...
    try {
        ...
        const isDuplicate = listCheck.some(item => 
            item.tenMauSac.toLowerCase() === form.ten.trim().toLowerCase() && item.id !== currentId.value
        );
...
const handleToggleStatus = async (item, event) => {
    ...
    if (result.isConfirmed) {
        try {
            ...
            else await axios.put(`${API_URL}/${item.id}`, { ten: item.tenMauSac, trangThai: 1 });

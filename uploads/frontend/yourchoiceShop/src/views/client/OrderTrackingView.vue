        ...
        <div class="modal-content">
          ...
          <div class="modal-body" v-if="orderDetail">
            ...
            <div class="detail-section">
              ...
              <p>{{ orderDetail.thongTinNhanHang?.soDienThoai || orderDetail.thongTinNhanHang?.sdt }}</p>
            ...
            <div class="detail-section">
              ...
              <div class="detail-product" v-for="(sp, idx) in getOrderProducts()" :key="sp.idHoaDonChiTiet || sp.maSanPham || idx">
                <img :src="resolveProductImage(sp)" alt="" @error="(e) => e.target.src = 'https://placehold.co/60x75?text=N/A'" />
                <div class="dp-info">
                  <h5>{{ sp.tenSanPham }}</h5>
                  <span>{{ sp.mauSac || '-' }} / {{ getProductSize(sp) }} x {{ sp.soLuong || 0 }}</span>
                </div>
            ...
            <div class="detail-section payment-summary">
              ...
              <div class="ps-row" v-if="(orderDetail.tienGiamGia || orderDetail.tienGiam)"><span>Giảm giá</span><span class="discount">-{{ formatMoney(orderDetail.tienGiamGia || orderDetail.tienGiam) }}</span></div>
...
const getOrderProducts = () => {
  ...
  const products = detail?.sanPhams ?? detail?.sanPhamHoaDon
  return Array.isArray(products) ? products : []
}

const resolveProductImage = (product) => {
  const rawImage = product?.duongDanAnh || product?.anh || product?.hinhAnh
  if (!rawImage) return 'https://placehold.co/60x75?text=N/A'
  if (rawImage.startsWith('http://') || rawImage.startsWith('https://')) return rawImage
  return `${IMAGE_BASE_URL}${String(rawImage).replace(/^\/+/, '')}`
}

const getProductSize = (product) => product?.kichThuoc || product?.size || '-'

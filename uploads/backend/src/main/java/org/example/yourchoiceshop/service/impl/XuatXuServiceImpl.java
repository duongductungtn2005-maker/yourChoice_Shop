...
public class XuatXuServiceImpl {
    ...
    // UPDATE: Thêm status
    public Page<XuatXu> getAll(String keyword, Integer status, Pageable pageable) {
        return repository.search(keyword, status, pageable);
    }

    public XuatXu getById(Integer id) {
        return repository.findById(id).orElseThrow(() -> new RuntimeException("Không tìm thấy Xuất xứ ID: " + id));
    }

    public XuatXu create(StoreAttributeRequest req) {
        XuatXu entity = new XuatXu();
        entity.setMaXuatXu("XX" + System.currentTimeMillis());
    ...
    public ByteArrayInputStream exportToExcel() throws IOException {
        List<XuatXu> dataList = repository.findAll();
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            String[] columns = {"ID", "Mã Xuất Xứ", "Tên Xuất Xứ", "Trạng Thái", "Ngày Tạo"};
            ...
            // 2. Data
            int rowIdx = 1;
            for (XuatXu item : dataList) {
                Row row = sheet.createRow(rowIdx++);

                row.createCell(0).setCellValue(item.getId());
                row.createCell(1).setCellValue(item.getMaXuatXu() != null ? item.getMaXuatXu() : "");
                row.createCell(2).setCellValue(item.getTenXuatXu());

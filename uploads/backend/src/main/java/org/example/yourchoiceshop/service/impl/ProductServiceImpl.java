...
public class ProductServiceImpl {
    ...
    public List<VariantResponse> getVariantsByProductId(Integer productId) {
        ...
        return entities.stream().map(this::mapToVariantResponse).collect(Collectors.toList());
    }

    @Transactional
    public VariantResponse createVariantForProduct(Integer productId, ProductVariantRequest req) {
        SanPham parent = sanPhamRepo.findById(productId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy sản phẩm ID: " + productId));

        if (req.getIdMauSac() == null || req.getIdKichThuoc() == null) {
            throw new RuntimeException("Màu sắc và kích thước là bắt buộc");
        }
        ...
        boolean duplicated = chiTietRepo.existsBySanPhamIdAndMauSacIdAndKichThuocId(
                productId,
                req.getIdMauSac(),
                req.getIdKichThuoc()
        );
        if (duplicated) {
            throw new RuntimeException("Biến thể màu sắc/kích thước này đã tồn tại");
        }

        MauSac mauSac = mauSacRepo.findById(req.getIdMauSac())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy màu sắc ID: " + req.getIdMauSac()));
        KichThuoc kichThuoc = kichThuocRepo.findById(req.getIdKichThuoc())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy kích thước ID: " + req.getIdKichThuoc()));

        ChiTietSanPham variant = new ChiTietSanPham();
        variant.setSanPham(parent);
        variant.setMauSac(mauSac);
        variant.setKichThuoc(kichThuoc);
        variant.setThuongHieu(parent.getThuongHieu());
        variant.setChatLieu(parent.getChatLieu());
        variant.setCoAo(parent.getCoAo());
        variant.setTayAo(parent.getTayAo());
        variant.setXuatXu(parent.getXuatXu());
        variant.setMaCtsp(generateUniqueVariantCode(parent.getMaSanPham(), req.getIdMauSac(), req.getIdKichThuoc()));
        variant.setGiaNhap(req.getGiaNhap());
        variant.setGiaBan(req.getGiaBan());
        variant.setSoLuong(req.getSoLuong());
        variant.setTrangThai(parent.getTrangThai() != null ? parent.getTrangThai() : 1);

        ChiTietSanPham savedVariant = chiTietRepo.save(variant);

        if (req.getListAnh() != null && !req.getListAnh().isEmpty()) {
            ...
            for (String url : req.getListAnh()) {
                ...
                HinhAnh img = new HinhAnh();
                img.setChiTietSanPham(savedVariant);
                img.setDuongDanAnh(url.trim());
                img.setTenAnh("Add-" + savedVariant.getMaCtsp());
                img.setTrangThai(1);
                newImages.add(img);
            }
        ...
        return mapToVariantResponse(chiTietRepo.findById(savedVariant.getId()).orElse(savedVariant));
    }
    ...
    private VariantResponse mapToVariantResponse(ChiTietSanPham ct) {
    ...
    private String generateUniqueVariantCode(String maSanPham, Integer idMauSac, Integer idKichThuoc) {
        String baseCode = maSanPham + "-" + idMauSac + "-" + idKichThuoc;
        if (!chiTietRepo.existsByMaCtsp(baseCode)) {
            return baseCode;
        }

        int suffix = 1;
        String candidate = baseCode + "-" + suffix;
        while (chiTietRepo.existsByMaCtsp(candidate)) {
            suffix++;

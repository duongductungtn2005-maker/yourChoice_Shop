...
public class StatisticRepositoryImpl implements StatisticRepository {
    ...
    public RevenueSummaryDTO getRevenueSummary(StatisticFilterRequest filter) {
        StringBuilder sql = new StringBuilder(
                "SELECT " +
                // index 0: Thực tế (Trạng thái hoàn thành: mới = 4, cũ = 5)
                "  COALESCE(SUM(CASE WHEN hd.trang_thai IN (4,5) THEN (hd.tong_tien_sau_giam - COALESCE(hd.phi_van_chuyen, 0)) ELSE 0 END), 0) AS totalRevenue, " +
                // index 1: Dự kiến (Không tính đơn hủy/trả)
                ...
                "  COALESCE(SUM(CASE WHEN hd.trang_thai IN (4,5) THEN 1 ELSE 0 END), 0) AS successOrders, " +
                "  COALESCE(SUM(CASE WHEN hd.trang_thai IN (1,2,3) THEN 1 ELSE 0 END), 0) AS processingOrders, " +
        ...
        StringBuilder sqlProducts = new StringBuilder(
                ...
                "JOIN hoa_don hd ON hdct.id_hoa_don = hd.id WHERE hd.trang_thai IN (4,5) "
        );
    ...
    public List<EmployeeStatDTO> getEmployeeStats(StatisticFilterRequest filter) {
        StringBuilder sql = new StringBuilder(
                "SELECT " +
                        ...
                        "WHERE ((:status IS NULL AND hd.trang_thai IN (4,5)) OR (:status IS NOT NULL AND hd.trang_thai = :status)) "
        );
